...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self.tabBarController.navigationItem setTitle:@"Done Screen"];
}
...
- (void)update {
    [self fetchDoneTasks];
    _doneTasks.count > 0 ? [_noTasksImage setHidden:YES] : [_noTasksImage setHidden:NO];
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if (_isFiltered) {
        ...
        currentTask = [_doneTasks objectAtIndex:indexPath.row];
    }
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        UIAlertAction *delete = [UIAlertAction actionWithTitle:@"Delete" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            ...
            if (self->_isFiltered) {
                ...
                taskToDelete = self->_doneTasks[indexPath.row];
            }
            
            [UserDefaultManager deleteTaskWithId:taskToDelete.taskID];
            [self->_doneTasks removeObject:taskToDelete];
            self->_doneTasks.count > 0 ? [self->_noTasksImage setHidden:YES] : [self->_noTasksImage setHidden:NO];
...
- (NSArray *)getFilteredTasks:(NSInteger)section {
    ...
    NSArray *filteredTasks = [_doneTasks filteredArrayUsingPredicate:predicate];
